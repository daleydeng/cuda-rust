...
impl DeviceMemory {
	...
	ffi::cudaMemcpy(self.ptr, src, count, ffi::cudaMemcpyKind::cudaMemcpyHostToDevice);
    }

    pub unsafe fn copy_to_host(&self, dst: *mut c_void, count: usize) {
	assert_le!(count, self.size);
	ffi::cudaMemcpy(dst, self.ptr, count, ffi::cudaMemcpyKind::cudaMemcpyDeviceToHost);
    }
